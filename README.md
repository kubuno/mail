...
<p align="center">
  <img src=".github/logo.png" alt="Kubuno Mail logo" width="128" height="128">
</p>
...
- **Fast composing** — floating compose window (with a full-screen mode) and inline reply/forward with rich-text formatting (web-safe font families, arbitrary pixel sizes), To / Cc / Bcc fields, scheduled send, undo send and drafts. Replies and forwards embed the quoted original in the editor, ready to be trimmed or annotated.
- **New-message menu** — the compose button offers, besides a plain message: an **encrypted (OpenPGP)** message, a message **from a saved template** (server-stored, plus "save as template" from the composer), and quick actions to create a **label**, a **filter/rule**, a **personal distribution list**, or a **scheduled** message.
- **Gmail-parity attachments** — full-width attachment bands with per-file encode progress and cancel, **drag-and-drop** of files onto the composer, in-place preview/download, and files exceeding the admin size limit are transparently uploaded to the user's **Drive** (`Mail/Attachment/`) and sent as a public link instead of a bounced oversized message.
- **Contact @mentions** — typing `@` in the message body suggests contacts (provided by the Contacts module, dynamically discovered) and inserts them as a removable chip, serialized as a `mailto:` link when the mail is sent so any recipient — Kubuno user or not — gets a clickable name.
- **Signatures** — several named signatures with a rich-text editor, a default per **sending address** (distinct for new messages vs replies/forwards), auto-inserted in the composer.
...
- **Automation & delegation** — a vacation **auto-responder** (RFC 3834 loop-safe), real message **forwarding** rules, verified **send-as** identities (ownership confirmed by email), account **delegation** (a delegate reads and sends *on behalf of* the grantor, with a proper `Sender:` header), and per-user **POP/IMAP** access controls enforced by the instance's own IMAP/POP3 server.
